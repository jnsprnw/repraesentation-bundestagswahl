...
<script>
  ...
  const { width, height, xScale, yRange, data } = getContext('LayerCake');
  ...
  {#each tickVals as tick, i (tick)}
    ...
    <g class="tick tick-{i}" transform="translate({$xScale(tick)},{Math.max(...$yRange)})">
      ...
      {#if tickMarks === true}
        ...
        />
      {/if}
      {#if $data.find((d) => d.party === tick)?.is_include}
        <text
          style="transform-origin: {halfBand}px {tickGutter + tickLen}px"
          class="rotate-35 fill-gray-500 text-xs"
          x={halfBand}
